...
function Update-Tree {
    ...
    Move-Cursor $Tree.Position
    $colour = $PSStyle.Foreground.White
    if ($Working) {
        $colour = $PSStyle.Foreground.Yellow
    } elseif ($Tree.IsVisible) {
...
do {
    ...
    foreach ($rotate in 'Clockwise', 'Anticlockwise') {
        ...
        Move-Cursor 0, ($data.Count + 5)
        [Console]::WriteLine("Visible trees: $result")
    }
