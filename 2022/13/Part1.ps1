...
$data = [System.IO.File]::ReadAllLines("$PSScriptRoot\input.txt") | Where-Object Length

for (($i = 0), ($index = 1), ($sum = 0); $i -lt $data.Count; ($i += 2), ($index++)) {
    $left, $right = $data[$i, ($i + 1)] -as [Packet[]]
    if ($left -lt $right) {
        $sum += $index
    }
}
