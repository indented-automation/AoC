...
$segmentRules = @(
    ...
    @{
        Name       = 'a'
        Rule       = { $numbers.Contains(4) -and $numbers.Contains(7) }
        Expression = { $numbers - ($numbers[4] -band $numbers[7]) }
    }
    @{
        Name       = 'b'
        Rule       = { $numbers.Contains(3) -and $numbers.Contains(5) }
        Expression = { $numbers[5] - ($numbers[5] -band $numbers[3]) }
    }
    @{
        Name       = 'b'
    ...
    @{
        Name       = 'c'
        Rule       = { $segments.Contains('f') }
        Expression = { $numbers[1] -bxor $segments['c'] }
    }
    @{
        Name       = 'd'
...
$numberRules = @(
    @{
        Value = 0
        Rule  = {
            if ($value.Length -eq 6 -and $segments.Contains('c') -and $segments.Contains('e')) {
    ...
    @{
        Value = 0
        Rule  = {
            $value.Length -eq 6 -and
            $numbers.Contains(6) -and $numbers.Contains(9)
        }
    }
    @{
        Value = 2
        Rule  = {
            $value.Length -eq 5 -and
    ...
    @{
        ...
        Rule = {
            $value.Length -eq 5 -and
            $segments.Contains('e') -and
            ($valueData.Bits -band $segments['e']) -eq $segments['e']
        }
    }
    @{
        Value = 3
        Rule  = {
            $value.Length -eq 5 -and
            $numbers.Contains(2) -and $numbers.Contains(5)
        }
    }
    @{
        Value = 3
    ...
    @{
        ...
        Rule  = {
            $value.Length -eq 5 -and
            $numbers.Contains(2) -and $numbers.Contains(3)
        }
    }
    @{
        Value = 5
...
foreach ($signalPattern in $signalPatterns) {
    ...
    do {
        ...
        foreach ($value in [string[]]$values.Keys) {
            ...
            :rule foreach ($rule in $numberRules) {
                if (& $rule.Rule) {
